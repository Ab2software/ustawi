...
const Forgot = (props) => {
  ...
  const SubmitLoginData = async () => {
    ...
    //     body.append('country_code', country);
    ...
    //       // body.append('mobile', mobile);
    //       setAnimating(true);
    //       let result = await apiImageUpload(url, body, false)
    ...
    //       }
    //     } else {
    //       Toast.show({ type: 'error', text1: "Your Mobile number not register" });
    //       setAnimating(false);
    //     }
    //   } catch (error) {
    //     setAnimating(false);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: currentTheme().background }}>
      ...
      <View style={{ marginHorizontal: 10, paddingHorizontal: 10 }}>
        ...
        <View style={{ marginTop: 100 }}>
          <Text style={{ color: currentTheme().text, fontSize: 26, fontWeight: 'bold' }}>Forgot Password</Text>
            ...
            <View style={{ flex: 0.8, borderColor: currentTheme().text, justifyContent: 'center' }}>
              ...
              keyboardType='number-pad'
               ...
               placeholderTextColor={currentTheme().placeholdercolor}
                style={{ paddingLeft:20,borderLeftWidth: 0.5, borderColor: currentTheme().text, height: 40, color: currentTheme().text }} />
          ...
          />
          <View style={{ marginTop: 10, alignItems: 'center', flexDirection: 'row' ,marginVertical:30}}>
          ...
          <TouchableOpacity style={{ borderWidth: 0.5, height: 43, justifyContent: 'center', alignItems: 'center', borderRadius: 10, marginTop: 30, marginHorizontal: 50, borderColor: currentTheme().text }}>
            <Text style={{ color: currentTheme().text }}>Sign up with Equity bank</Text>
          </TouchableOpacity> */}
        </View>
      </View>
